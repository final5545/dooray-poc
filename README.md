...
| | 어디서 | 무엇을 |
|---|---|---|
| **고객정보 조회** | `CRM 조회` 방 | 고객번호를 섞어 쓰면 고객 카드가 온다 (사내 CRM 실 API) |
| **자연어 요청** | `CRM 조회` 방 | "E120452 이전 요청할래" → 양식을 대신 채워 확인 |
...
- **CRM은 사내 실 API를 씁니다**(2026-09-10 연동). 다만 계약정보·설치정보는
  응답에 없어 그 섹션이 화면에서 생략됩니다.
- 모의 서버로 되돌리려면 `.env`에 `CRM_KIND=http` · `CRM_BASE_URL=http://127.0.0.1:8900`
- ⚠️ **서버(ai-node)에서는 CRM에 닿지 않습니다.** `/접수` 로 만드는 업무 제목에
  회사명이 빠질 뿐이라 양식의 「고객명」 칸을 채우면 됩니다. 조회와 자연어
  접수는 맥에서 하므로 영향이 없습니다.
- **승인된 사용자만 쓸 수 있습니다**(`DOORAY_ALLOWED_USERS`). 비워 두면 제한하지
  않으니, 실 고객정보를 다루는 환경에서는 반드시 채우세요.
- 조회·생성·상태변경은 `DOORAY_AUDIT_LOG` 에 기록됩니다. **무엇을 조회했는지만
  남고 결과는 남지 않습니다.**
